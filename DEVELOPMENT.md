...
**File ▸ Back up planner** (and **Back up now** in Settings ▸ Your data) writes
a `.json` copy into the backup folder the person chose, asking for one the
first time; setup's last page asks for it too. The desktop app writes there
itself; in Chrome and Edge the page is given the folder through the File
System Access API. Automatic backups repeat daily, weekly or monthly. Only
where no folder can be chosen (Firefox, Safari) is a backup a download.
**Restore** reads a file back and replaces the planner, which is also how a
planner moves to another computer.
